...
class LearnSpaceSeeder extends Seeder
{
    ...
    {
        // 1. Créer le Parcours "Niveau Zéro"
        $parcours = Parcours::firstOrCreate(
            ['slug' => 'niveau-zero'],
            [
                'titre' => 'Niveau Zéro',
                'description' => 'Les bases absolues pour bien démarrer en allemand.',
                'difficulte' => 'Débutant',
                'couleur_theme' => '#4F46E5',
            ]
        );

        // 2. Créer les Chapitres et Leçons
        $ordreChapitre = 1;

        foreach (self::$dataset as $slug => $data) {
            $chapitre = Chapitre::firstOrCreate(
                ['slug' => $slug, 'parcours_id' => $parcours->id],
                [
                    'titre' => $data['titre'],
                    'sur_titre' => $data['sur_titre'],
                    'description' => $data['description'],
                    'template_vue' => $data['template'],
                    'couleur_theme' => $data['couleur_theme'],
                    'ordre' => $ordreChapitre++,
                ]
            );

            // Créer les leçons (mots) pour ce chapitre
            $ordreLecon = 1;
            foreach ($data['items'] as $item) {
                Lecon::updateOrCreate(
                    [
                        'chapitre_id' => $chapitre->id,
                        'mot_allemand' => $item['de'],
                    ],
                    [
                        'traduction_francaise' => $item['fr'] ?? null,
                        'exemple' => $item['example'] ?? null,
                        'chemin_audio' => $item['audio'] ?? null,
                        'code_couleur' => $item['hex'] ?? null,
                        'chiffre' => $item['num'] ?? null,
                        'ordre' => $ordreLecon++,
                    ]
                );
            }
