<style>
  ...
  .structure-card { background: var(--lab-surface); border: 2px solid var(--acc-brd); border-radius: 12px; padding: 20px; text-align: center; }
  .structure-label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: .1em; color: var(--lab-charcoal-light); margin-bottom: 12px; }
  .structure-title { font-family: 'Lora', serif; font-size: 18px; font-weight: 700; margin-bottom: 16px; color: var(--lab-ink); }
  .structure-items { display: flex; flex-direction: column; gap: 8px; }
  .structure-item { background: var(--lab-stone); padding: 10px 12px; border-radius: 8px; font-size: 13px; color: var(--lab-charcoal-mid); transition: all .2s ease; }
  .structure-item:hover { border-left-color: var(--acc); background: var(--acc-lt); }
  .structure-item-de { font-family: 'Lora', serif; font-weight: 600; color: var(--lab-ink); margin-top: 4px; }
  .structure-item-fr { font-size: 11px; color: var(--lab-charcoal-light); margin-top: 2px; }
  ...
  .example-cell { background: var(--lab-surface); padding: 12px; border-radius: 8px; border: 1px solid var(--acc-brd); }
  ...
  .example-cell-text { font-family: 'Lora', serif; font-size: 15px; font-weight: 600; color: var(--lab-ink); }
  .example-cell-fr { font-size: 11px; color: var(--lab-charcoal-light); margin-top: 4px; }

  /* ── SECTION LABEL ── */
  .section-label { display: flex; align-items: center; gap: 10px; font-size: 11px; font-weight: 600; letter-spacing: .1em; text-transform: uppercase; color: var(--lab-charcoal-light); padding: 28px 0 12px; }
  .section-label::after { content:''; flex:1; height:1px; background:var(--lab-glass-border); }

  /* ── CONJUGAISON TABLES ── */
  .cj-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
  @media (max-width: 560px) { .cj-grid { grid-template-columns: 1fr; } }
  .cj-card { border-radius: 12px; overflow: hidden; border: 1px solid var(--lab-glass-border); background: var(--lab-surface); }
  ...
  .cj-table thead th { font-size: 10px; font-weight: 600; letter-spacing: .07em; text-transform: uppercase; color: var(--lab-charcoal-light); background: var(--lab-stone); padding: 7px 12px; border-bottom: 1px solid var(--lab-glass-border); text-align: left; }
  .cj-row { cursor: pointer; transition: background .12s; }
  .cj-row:not(:last-child) td { border-bottom: 1px solid var(--lab-glass-border); }
  .cj-row td { padding: 9px 12px; vertical-align: middle; }
  .cj-pro-cell { width: 42%; border-right: 1px solid var(--lab-glass-border); }
  .cj-pro { font-size: 13px; color: var(--lab-charcoal-mid); }
  .cj-pro-fr { font-size: 11px; color: var(--lab-charcoal-light); margin-top: 1px; }
  ...
  .mini-play { width: 24px; height: 24px; border-radius: 50%; border: none; background: transparent; cursor: pointer; color: var(--lab-charcoal-light); display: flex; align-items: center; justify-content: center; transition: color .15s, transform .1s; padding: 0; margin-left: auto; font-size: 15px; }
  ...
  /* ── TRANSITION ── */
  .transition-block { margin: 28px 0 0; background: var(--lab-stone); border: 1px solid var(--lab-glass-border); border-left: 3px solid var(--acc); border-radius: 0 12px 12px 0; padding: 16px 18px; }
  .transition-block p { font-family: 'Lora', serif; font-size: 14px; font-style: italic; color: var(--lab-charcoal-mid); line-height: 1.7; }
  .transition-block strong { font-style: normal; color: var(--lab-ink); }

  /* ── PHRASE TABLE ── */
  .pl-outer { padding: 0; }
  .pl-verb-block { margin-bottom: 24px; background: var(--lab-surface); border-radius: 12px; }
  .pl-verb-title { display: flex; align-items: center; gap: 10px; padding: 14px 14px 10px; border-radius: 12px 12px 0 0; border: 1px solid var(--lab-glass-border); border-bottom: none; background: var(--acc-lt); color: var(--acc); }
  ...
  .pl-grid { display: grid; grid-template-columns: repeat(4, 1fr); border: 1px solid var(--lab-glass-border); border-radius: 0 0 12px 12px; overflow: visible; }
  @media (max-width: 900px) { .pl-grid { grid-template-columns: repeat(2, 1fr); } }
  @media (max-width: 500px) { .pl-grid { grid-template-columns: 1fr; } }

  .pl-item { display: flex; align-items: center; justify-content: space-between; padding: 8px 8px 8px 10px; gap: 5px; cursor: pointer; transition: background .12s; border-bottom: 1px solid var(--lab-glass-border); }
  .pl-item:not(:nth-child(4n)) { border-right: 1px solid var(--lab-glass-border); }
  @media (max-width: 900px) { .pl-item:not(:nth-child(2n)) { border-right: 1px solid var(--lab-glass-border); } .pl-item:nth-child(2n) { border-right: none; } }
  
  .pl-text { flex:1; min-width:0; pointer-events:none; }
  .pl-fr { font-size: 12.5px; color: var(--lab-charcoal-light); margin-bottom: 2px; }
  ...
  .act-play { width: 26px; height: 26px; border-radius: 50%; border: none; background: transparent; cursor: pointer; color: var(--lab-charcoal-light); display: flex; align-items: center; justify-content: center; font-size: 15px; transition: color .15s, transform .1s; padding: 0; }
  ...
  .mini-tooltip {
    ...
    background: var(--lab-surface); border:0.5px solid var(--lab-glass-border); border-radius:12px;
  ...
  .mini-tooltip::after { content:''; position:absolute; top:100%; right:14px; border:7px solid transparent; border-top-color:var(--lab-glass-border); }
  .mini-tooltip::before { content:''; position:absolute; top:100%; right:15px; border:6px solid transparent; border-top-color:var(--lab-surface); z-index:1; }

  /* Tooltip content */
  .t-head { display:flex; align-items:center; gap:8px; margin-bottom:10px; padding-bottom:10px; border-bottom:0.5px solid var(--lab-glass-border); }
  ...
  .t-name { font-size:13px; font-weight:600; color:var(--lab-ink); line-height:1.2; }
  .t-role { font-size:11px; color:var(--lab-charcoal-light); }

  .t-score-row { display:flex; align-items:center; gap:8px; margin-bottom:8px; }
  .t-score-val { font-size:20px; font-weight:700; }
  
  .t-bar-track { height:4px; background:var(--lab-stone); border-radius:2px; flex:1; overflow:hidden; }
  ...
  .t-text { font-size:12px; color:var(--lab-charcoal-mid); line-height:1.5; margin-bottom:10px; text-align: left; white-space: normal; }
  .t-text strong { font-weight:600; color:var(--lab-ink); }
  ...
  /* ── NAVIGATION ── */
  .lesson-nav { display: flex; justify-content: space-between; align-items: center; gap: 16px; margin: 40px 0 0; padding-top: 24px; border-top: 1px solid var(--lab-glass-border); }
